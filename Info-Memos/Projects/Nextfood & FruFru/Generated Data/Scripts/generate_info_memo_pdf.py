...
def create_info_memo_pdf():
    ...
    frufru_text = (
        ...
        'Distributed via OTOTO stores (Simiuc\'s separate retail chain), Freshful, Kaufland, Mega Image, Carrefour, and lifebox.ro.'
    )
    lifebox_text = (
    ...
    elements.append(Paragraph(
        ...
        '<b>Distribution:</b> Freshful, Kaufland, Mega Image, Carrefour, lifebox.ro, and OTOTO stores (Simiuc\'s separate retail business).',
        body_small_style
    ))
    elements.append(Spacer(1, 0.1 * cm))
    ...
    kpi_boxes = Table([[
        ...
        make_kpi_box("CAGR 24→26E", fmt_pct(cagr_rev), GREEN_ACCENT),
    ...
    elements.append(Paragraph("Consolidated P&L — Lifebox + Catering (2023–2024) | + FruFru (2025B)", subsection_style))

    c = fin['consolidated']
    c25 = fin['consolidated_2025']
    growth_rev = fin['growth_rev_2023_2024']

    # Common metrics mapped across both CSVs
    # consolidated (2023, 2024): Net_Revenue, Material_Costs, Gross_Margin, Labor_Direct_Indirect,
    #   Manufacturing, Transport_Costs, Marketing, GA, EBITDA_Normalised, Net_Profit
    # consolidated_2025: Sales, COGS, Gross_Margin_After_COGS, Labor_Direct_Indirect,
    #   Manufacturing, Delivery, Marketing, GA, EBITDA, Net_Profit

    # Gross_Margin_After_COGS (2025) = Sales - COGS = Material Margin (same as Material_Margin in 2023-2024)
    # Gross_Margin (2023-2024) = Material Margin - Labor - Manufacturing - Depreciation Production
    ...
    cw3 = (page_width - 4 * cm) / 3
    # Bold rows: Net Revenue (1), Material Margin (3), Gross Margin (7), EBITDA (11), Net Profit (12)
    elements.append(make_table(merged_table_data,
        col_widths=[4 * cm, cw3, cw3, cw3],
        bold_rows={1, 3, 7, 11, 12}))
    elements.append(Spacer(1, 0.05 * cm))

    elements.append(Paragraph(
        f'<i>2023–2024: Lifebox + Catering only. 2025B includes FruFru relaunch. '
        f'Revenue grew {fmt_pct(growth_rev)} YoY (2023→2024). '
        f'EBITDA negative due to relaunch investment and scaling costs.</i>',
        small_style
    ))
    ...
    highlights = [
        ...
        ('<b>Multi-Channel Distribution.</b> Modern trade (Mega Image, Carrefour, Freshful, Kaufland), '
    ...
    portfolio_data = [
        ...
        ['FruFru', 'Retail (Mega Image, Carrefour,\nFreshful, Kaufland, OTOTO*)', '~30+ SKUs: salads, soups,\nbowls, mains, desserts',
         'RON 15–41\n(retail packs)'],
